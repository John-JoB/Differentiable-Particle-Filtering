...
def sinkhorn_loop(log_a: pt.Tensor, log_b: pt.Tensor, cost: pt.Tensor, epsilon: float, threshold: float, max_iter: int, diam: pt.Tensor, rate: float):
    """
...
def get_sinkhorn_inputs_OT(log_weights: pt.Tensor, N: int, x_t: pt.Tensor):
    """
...
class OT_Resampler(Resampler):
    ...
    def __init__(self, epsilon, threshold, max_iter, rate):
        super().__init__()
    ...
    def forward(self, x_t:pt.Tensor, log_w_t:pt.Tensor):
        N = x_t.size(1)
        log_b, cost, diam = get_sinkhorn_inputs_OT(log_w_t, N, x_t)
        diam = x_t.max(dim=1)[0].max(dim=1)[0] - x_t.min(dim=1)[0].min(dim=1)[0]
        f, g, epsilon_used = sinkhorn_loop(log_w_t, log_b, cost, self.epsilon, self.threshold, self.max_iter, diam.reshape(-1, 1, 1), self.rate)
...
def batched_reindex(vector:pt.Tensor, indicies:pt.Tensor):
    ...
    scaled_indicies = (indicies + pt.arange(shape[0], device=device).unsqueeze(dim=1)*shape[1]).to(pt.int).view(-1)
...
class soft_grad_wrapper(pt.autograd.Function):
    ...
    """
    @staticmethod
    def forward(ctx:Any, particle_probs:pt.Tensor, norm_w_t:pt.Tensor, new_weights:pt.tensor):
        ctx.save_for_backward(particle_probs, norm_w_t, new_weights)
        return new_weights.clone()
    
    @staticmethod
    def backward(ctx, d_dweights):
        d_dweights = pt.clamp(d_dweights, -100., 100.)
        particle_probs, norm_w_t, new_weights = ctx.saved_tensors
        d_dp, d_dw = pt.autograd.grad(new_weights, [particle_probs, norm_w_t], grad_outputs=d_dweights, retain_graph=True)
        return d_dp, d_dw, None
...
class Soft_Resampler_Systematic(Resampler):
    ...
    def __init__(self, tradeoff:float):
        super().__init__()
        self.rng = pt.Generator(device = device)
    ...
    def forward(self, x_t:pt.Tensor, log_w_t:pt.Tensor):
        ...
        if self.tradeoff == 1:
            ...
            log_particle_probs = pt.logaddexp(self.log_tradeoff+log_w_t,  self.log_inv_tradeoff - pt.log(pt.tensor((N), device=device)))
        
        offset = pt.rand((B), generator=self.rng, device=device)
        ...
        resampling_points = pt.arange(N, device=device).unsqueeze(dim=0) + offset.unsqueeze(dim=1)
        ...
        if self.tradeoff == 1.:
            new_weights = pt.zeros_like(log_w_t, device= device)
...
class Soft_Resampler_Multinomial(Resampler):
    ...
    def __init__(self, tradeoff:float, ratio:float):
        super().__init__()
        self.rng = pt.Generator(device = device)
        self.tradeoff = tradeoff
        self.ratio = ratio
    ...
    def forward(self, x_t:pt.Tensor, log_w_t:pt.Tensor):
        ...
        resampled_indicies = pt.multinomial(particle_probs, N//self.ratio, replacement=True, generator=self.rng).to(device=device)
        resampled_particles = batched_reindex(x_t, resampled_indicies)
        if self.tradeoff == 1.:
            new_weights = pt.zeros_like(log_w_t,device=device)
        else:
